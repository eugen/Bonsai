...
Primitivele în Bonsai sunt extensii ale anumitor tipuri, similar cu extension methods din C\# 3.0. De exemplu, primitiva \c{capitalize} poate fi privită ca o extensie a tipului \c{System.String}, iar \c{shuffle} o extensie a tipului \c{System.Collections.Generic.IList<T>}.
...
\begin{code}
\hspace[Primitive(typeof(string), "capitalize")]
...
Primitivele pot fi definite și pentru tipuri CLR generice: o primitivă definită pentru tipul generic \c{typeof(List<>)} se va aplica tuturor instanțierilor acestui tip. Primitiva \c{shuffle} este un exemplu:
\begin{code}
\hspace[Primitive(typeof(IList<>), "shuffle")]
...
Din exemplele de mai sus se poate observa că metodele ce implementează primitive trebuie să aibă trei parametri:
\begin{description}
...
Capitolul \ref{ch:Primitives} include lista completă de primitive din Bonsai.
...
Cea mai mare parte din semantica Bonsai este implementată de clase care moștenesc \c{BonsaiFunction}. Lista completă este inclusă în capitolul \ref{ch:BonsaiFunction}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Funcții standard}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

În secțiunea \ref{sec:ScopeManagement} a fost menționat că lista variabilelor definite (contextul curent) este stocată într-o instanță a clasei \c{DictionaryBonsaiFunction} și că aceasta conține întotdeauna o referință către contextul părinte. 
Acest lucru este în continuare adevărat, cu o singură excepție: contextul din rădăcină, care nu are părinte, este o instanță a clasei \c{PreludeFunction}.

Clasa \c{PreludeFunction} moștenește clasa \c{DictionaryBonsaiFunction}. Funcționalitatea adusă în plus e că, în constructor, contextul este prepopulat cu funcțiile standard din Bonsai.

Acest capitol enumerează aceste funcții standard și încearcă să ofere exemple de utilizare pentru fiecare dintre ele.
...
\section{Definirea funcțiilor cu \c{defun}}

Funcția \c{defun} este folosită pentru definirea de noi funcții. Parametrii acesteia sunt un simbol ce reprezintă numele funcției, urmat de N simboluri care conțin numele parametrilor, iar la final un block care constituie corpul noii funcții:
\begin{code}
...
prima este că o parte din argumente, în afară de simboluri reprezentând numele parametrilor, pot fi șabloane. Cea de-a doua este că apelările ulterioare cu același nume de funcție nu vor suprascrie funcția în context, ci vor adăuga șabloanele noi la funcția deja existentă.

Un șablon este o instanță a care implementează interfața \c{IPattern}:
\begin{code}
...
Funcția \c{if} primește 3 parametri: o valoare de test, un block sau o valoare pentru cazul True și un block sau o valoare pentru cazul False. 
...
După testare, dacă argumentul corespunzător rezultatului testului este o valoare atunci aceasta este returnată. Dacă este un block, atunci este evaluat și rezultatul evaluării blocului este returnat.
...
\draft{TODO: definește funcțiile when și unless}
...
\item[\c{print}]\hfill\\ Funcția \c{print} afișează toate argumentele în consolă. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Obiecte Bonsai}\label{ch:BonsaiFunction}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Symboluri}\label{sec:Symbols}

\draft{TODO: move or remove; this section does not belong here}


\draft{Un simbol din Bonsai este foarte asemănător simbolurilor din Ruby\cite{ruby_symbols} și Lisp\cite{clhs_symbols}. Un simbol se comportă de cele mai multe ori ca un string. Diferența majoră constă în faptul că pentru orice string există o singură instanță a unui simbol cu acel string, ceea ce face compararea a două simboluri foarte rapidă. De asemenea, utilizarea unui simbol indică de cele mai multe ori folosirea metaprogrămarii. Tipul la runtime al simbolurilor este \c{Microsoft.Scripting.Dynamic.SymbolId}.}

\section{Blocuri}

Un block în Bonsai reprezintă o secvență de instrucțiuni delimitată între acolade. Un block, ca orice alt obiect, poate fi stocat într-o variabilă, trimis ca argument către alte funcții sau apelat:
\begin{code}
= .a { 42 }
print (a .Invoke)
  -> 42
\end{code}
Metoda din fragmentul de mai sus, \c{Invoke}, este singura suportată de block-uri și nu are nici un parametru. 

Tipul la runtime al unui bloc este \c{BlockBonsaiFunction}, clasă derivată din \c{BonsaiFunction}. 
...
\item \c{.*} — înmulțire
\item \c{./} — împărțire
\item \c{.\textasciicircum} — ridicare la putere
\draft{
\item \c{.\%} — modulo
\item \c{.round} — rotunjire la cel mai apropiat întreg
\item \c{.floor} — rotunjire la întregul imediat inferior
\item \c{.ceiling} — rotunjire la întregul imediat superior
}
\end{itemize}

Toate constantele numerice din codul sursă Bonsai sunt parsate într-un singur tip de numere: raționale, reprezentate in virgulă fixă. Acesta mapează peste tipul \c{System.Decimal} din .NET Framework. Motivația o reprezintă elimitarea erorilor cauzate de reprezentarea in virgulă mobilă:
\begin{code}
// C#
float a = 1000f;
float b = 0.0001f;
...
Prototipul din care moștenesc toate obiectele specifice bonsai este o instanță a clasei \c{BonsaiPrototypeFunction}, stocată în variabila \c{object} din contextul rădăcină. Aceasta conține metodele comune tuturor obiectelor: \c{clone}, \c{=} și \c{method}.
...
Obiectului clonat i se pot adăuga metode noi prin apelarea \c{method}. Aceasta primește ca prim argument numele metodei ce urmeaza a fi definită, urmat de numele parametrilor metodei și, la final, blocul asociat metodei. În interiorul acestul bloc obiectul curent este conținut în variabila \c{self}.
...
Notă: după clonare, obiectul nou creat nu păstreaza legătura cu prototipul. Astfel, adăugarea metodelor sau câmpurilor într-un prototip nu se va reflecta și în obiectele clonate din acesta.

\section{Principiile OOP în programarea bazată pe prototipuri}

\subsection{Moșteneria}

Moștenirea într-un sistem OOP bazat pe prototipuri este reprezentată de clonarea obiectelor: clonele moștenesc metodele și câmpurile definite în prototip. De exemplu, în fragmentul de mai sus, obiectul \c{rect} moștenește metoda \c{area} de la prototipul \c{RectangleProto}.
...
\end{code}
\c{loadAssembly} poate fi utilizată pentru încărcarea unui assembly non-standard, precum module proprii sau alte biblioteci open-source.

\section{Funcția \c{import}}

Funcția \c{import} este folosită pentru importul spațiilor de nume și al claselor și are prototipul:
\begin{code}
...
% TODO: Do the following sections make sense here?
...
\begin{description}
\item[\c{getSourceCode .object}] Întoarce codul sursă pentru un obiect Bonsai.
\item[\c{getCurrentContext}] Întoarce contextul curent. Folosit pentru recompilarea unui fragment de code.
\item[\c{compileCode .codeFragment .scope}] Compilează un fragment de cod în contextul specificat de argumentul \c{scope}. 
\end{description}
...
% TODO: completely remove this section ?
% \subsection{Debugger}
% posibilități de realizare a unui debugger; integrarea cu VisualStudio?
